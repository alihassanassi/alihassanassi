...
The published architectural writing lives at alihasanassi.com. Four papers so far, plus a quarterly companion that names the gaps between what the papers claim and what the system does.
...
Persistent behavior layer. Operator-state modeling, persona-state tracking, interruption governance, cross-session memory, and an inner-voice daemon. Internally referred to as the consciousness layer. Not ornamental — these govern when JARVIS speaks, what it raises unprompted, and how it remembers the operator between sessions.
...
Research
Four papers so far at alihasanassi.com/research, plus a quarterly companion that names what the papers got wrong.
